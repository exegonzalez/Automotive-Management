...
import { editTransfer, getTransfer } from '../../../utils/functionsdb'
...
export default function EditTransfer() {
    ...
    useEffect(() => {
        const fetchTransfer = async control_number => {
            try{
                const transferFetched = await getTransfer(control_number)
                return transferFetched.transfer
                    ? ( setTransfer(transferFetched.transfer),
                      setVehicle(transferFetched.transfer.vehicle),
                      (transferFetched.transfer.clients.length === 1) 
                        ? ( setClient(transferFetched.transfer.clients[0]), setChecked(false) )
                        : ( setClient(transferFetched.transfer.clients[0]), setClient2(transferFetched.transfer.clients[1]), setChecked(true) )
                      )
    ...
    const handleSubmit = async e => {
        try {
            ...
            return await editTransfer(transfer)
                .then(handleStatus(true, 'success', '¡Transferencia editada exitosamente! :)'))
        ...
        <div className={classes.root}>
            ...
            <Grid container alignItems="center" justify="center">
                <Paper elevation={5} className={classes.paper}>
                    <h1 className={classes.h1}> <DescriptionIcon/> &bull; EDITAR TRANSFERENCIA &bull; <DescriptionIcon/> </h1>
                    <Divider/>
                    <form onSubmit={handleSubmit}>
                        ...
                        <Grid container spacing={3}>
                            ...
                            <Grid item xs={2}>
                                <Button type="submit" size="large" fullWidth="true" variant="contained" color="primary" startIcon={<EditIcon />}>
                                    Editar 
                                </Button>
                            </Grid>
                        </Grid>
